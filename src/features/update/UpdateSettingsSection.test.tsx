...
const status: UpdateState = {
  ...
  latestVersion: "1.0.5",
  channel: "stable",
  assetName: "ecology-section-archive_1.0.5_macos_aarch64_app.zip",
  assetPath: null,
  assetSha256: "abc",
  assetSize: 12345678,
  source: "github",
...
describe("UpdateSettingsSection", () => {
  ...
  test("renders install in-progress details after launching the helper", () => {
    const scheduledStatus: UpdateState = {
      ...status,
      status: "installing",
      assetPath: "/tmp/ecology-section-archive_1.0.5_app.zip",
  ...
  test("uses the latest channel when deriving optimistic download state without prior status", () => {
    const nextStatus = deriveDownloadProgressState(
      ...
      {
        version: "1.0.6",
        assetName: "ecology-section-archive_1.0.6_macos_aarch64_app.zip",
