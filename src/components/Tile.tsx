...
export function Tile({
  ...
  const { borderColor, cornerColor, titleColor, contentColor, emptyColor } = useMemo(() => {
    const isLightBg = chroma(tileColor).luminance() > 0.5;
    const mixTarget = isLightBg ? "#0d241e" : "#eaf6f0";
    return {
  ...
  return (
    <div
      {...divProps}
      className={`app-surface-frame relative border overflow-hidden select-none shadow-[0_1px_8px_rgba(11,60,50,0.08)] hover:shadow-[0_6px_24px_rgba(15,154,138,0.18)] ${className}`}
      style={mergedStyle}
    >
