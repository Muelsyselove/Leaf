...
import { EcoBrackets } from "./EcoDeco";
...
export function NotePad({
  ...
  const padSurfaceClassName =
    "app-surface-frame relative noise-bg w-full h-full min-h-0 eco-glass eco-clip-card overflow-hidden flex flex-col flex-1 border border-paper-deep/70 shadow-[0_1px_10px_rgba(11,60,50,0.1)] transition-all duration-200 ease-out";
    ...
    <div className={surfaceWrapperClassName}>
      {isTile ? (
        ...
        <div className={padSurfaceClassName} data-surface-mode={surfaceMode}>
          <>
            <EcoBrackets size={12} inset={6} className="opacity-60 z-10" />
            <div
            ...
            >
              <div className="flex items-center gap-0.5">
                ...
                >
                  ...
                  {mode === "new" && (
                    <div className="absolute bottom-0 left-3 right-3 h-[2px] bg-bamboo shadow-[0_0_8px_rgba(15,154,138,0.6)]" />
                  )}
                </button>
                <button
              ...
              <div className="ml-auto flex items-center gap-1.5">
                <button
                  ...
                  className="group w-7 h-7 flex items-center justify-center eco-clip-sm transition-all duration-200 cursor-pointer text-ink-ghost hover:text-ink-faint hover:bg-paper-warm"
                ...
                <button
                  onClick={() => void handleClose()}
                  className="group w-7 h-7 flex items-center justify-center eco-clip-sm text-ink-ghost hover:bg-danger-bg hover:text-red-400 transition-all duration-200 cursor-pointer"
            ...
            <div className="mx-4 mt-1 h-px bg-gradient-to-r from-transparent via-bamboo/45 to-transparent" />
            ...
            {mode === "new" ? (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-auto pt-2 border-t border-eco-teal/20 shrink-0">
                  <span className="eco-label text-[11px] text-ink-ghost font-mono tabular-nums truncate max-w-[170px]">
                    {`${countNoteChars(content)} ${t("common.wordCountUnit", { defaultValue: "字" })} · ${statusLabel[status]}`}
                  </span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={resetDraft}
                      className="px-4 py-1.5 text-[12px] text-ink-faint hover:text-ink-soft eco-clip-sm hover:bg-paper-warm transition-all duration-200 cursor-pointer"
                    >
                      {t("notepad.button.clear", { defaultValue: "清空" })}
                    </button>
                    <button
                      onClick={() => void handleSave()}
                      className="px-4 py-1.5 text-[12px] text-cloud bg-bamboo hover:bg-bamboo-light eco-clip-btn transition-all duration-200 font-medium cursor-pointer shadow-[0_4px_16px_rgba(15,154,138,0.35)] hover:shadow-[0_6px_20px_rgba(15,154,138,0.5)]"
                    >
