...
pub(crate) fn default_config_dir() -> Result<PathBuf, AppError> {
    if let Ok(path) = env::var("ECOLOGY_SECTION_ARCHIVE_CONFIG_DIR") {
    ...
    if let Some(dir) = dirs::config_dir() {
        return Ok(dir.join("ecology-section-archive"));
    }
    Ok(env::current_dir()?.join("ecology-section-archive"))
}

fn default_data_dir() -> Result<PathBuf, AppError> {
    if let Ok(path) = env::var("ECOLOGY_SECTION_ARCHIVE_DATA_DIR") {
        let trimmed = path.trim();
        if !trimmed.is_empty() {
            return Ok(PathBuf::from(trimmed));
        }
...
impl NoteStore {
    ...
    pub fn load_config(&self) -> Result<AppConfig, AppError> {
        ...
        // self.data_dir 与之不同（如 ECOLOGY_SECTION_ARCHIVE_DATA_DIR 被改），尝试搬运旧数据
    ...
    // （典型为修改 ECOLOGY_SECTION_ARCHIVE_DATA_DIR 环境变量），把旧位置数据搬到新位置。
...
mod tests {
    ...
    fn test_root(name: &str) -> PathBuf {
        let base = std::env::var_os("ECOLOGY_SECTION_ARCHIVE_TEST_TEMP_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|| std::env::temp_dir().join("ecology-section-archive-rust-tests"));
  ...
  {"id":"id-2","title":"第二篇","fileName":"id-2_第二篇.md","category":"工作","createdAt":"2026-01-01T00:00:00Z","updatedAt":"2026-01-01T00:00:00Z","wordCount":3,"preview":"第二篇"}
    ...
    // 模拟 ECOLOGY_SECTION_ARCHIVE_DATA_DIR 改向新空目录：旧位置数据应自动迁移过来
    #[test]
    ...
    fn migrate_data_to_rejects_target_inside_current_data_dir() {
        ...
        let error = store
            .migrate_data_to(&data_dir.join("notes").join("ecology"))
