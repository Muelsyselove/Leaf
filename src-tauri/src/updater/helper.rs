...
#[cfg(target_os = "windows")]
pub const HELPER_BINARY_NAME: &str = "ecology-section-archive-update-helper.exe";
#[cfg(not(target_os = "windows"))]
pub const HELPER_BINARY_NAME: &str = "ecology-section-archive-update-helper";
...
const UPDATE_STAGE_PREFIX: &str = ".ecology-section-archive-update-stage-";
#[cfg(target_os = "macos")]
const MOUNT_STAGE_PREFIX: &str = ".ecology-section-archive-mounted-dmg-";
...
fn windows_rollback_storage_parent(
    ...
    Ok(std::env::temp_dir().join("ecology-section-archive-update-rollback"))
}
...
fn unique_temp_path(parent: &Path, stem: &str, extension: Option<&str>) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let mut file_name = format!(".ecology-section-archive-{stem}-{unique}");
...
fn write_log_header(
    ...
    write_log_line(file, "ecology-section-archive update helper")?;
...
mod tests {
    ...
    fn temp_dir(name: &str) -> TestDir {
        ...
        let path = std::env::temp_dir()
            .join("ecology-section-archive-updater-tests")
    ...
    fn parses_strict_arguments() {
        let args: Vec<OsString> = vec![
            ...
            OsString::from("42"),
            OsString::from("--target-path"),
            OsString::from("/Applications/Ecology Section Archive.app"),
            OsString::from("--log-path"),
    ...
    fn expected_process_name_uses_macos_bundle_executable() {
        ...
        let bundle = root.join("生态科档案.app");
        let macos_dir = bundle.join("Contents").join("MacOS");
        fs::create_dir_all(&macos_dir).expect("create macos dir");
        fs::write(macos_dir.join("ecology-section-archive"), b"binary").expect("write binary");

        assert_eq!(
            expected_process_name(&bundle).as_deref(),
            Some("ecology-section-archive")
        );
        assert_eq!(
            expected_process_name(&root.join("ecology-section-archive")).as_deref(),
            Some("ecology-section-archive")
        );
    }
    ...
    fn cleanup_applied_update_retains_macos_rollback_stage() {
        ...
        let stage_root = root.join(".ecology-section-archive-update-stage-test");
    ...
    fn rollback_windows_update_restores_original_executable() {
        ...
        let target_path = install_dir.join("ecology-section-archive.exe");
        ...
        fs::write(
            backup_dir.join("ecology-section-archive.exe"),
            b"old version",
        )
        .expect("write backup");
    ...
    fn windows_rollback_storage_parent_moves_out_of_install_dir() {
        ...
        let mut command = helper_command(&root);
        command.install_kind = InstallKind::WindowsNsis;
        command.target_path = install_dir.join("ecology-section-archive.exe");
        command.ready_path = staging_dir.join("helper.ready");
        ...
        assert!(
            backup_parent.ends_with("ecology-section-archive-update-rollback"),
...
HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Uninstall\EcologySectionArchive
    DisplayIcon    REG_SZ    C:\Program Files\Ecology Section Archive\ecology-section-archive.exe,0
    InstallLocation    REG_SZ    C:\Program Files\Ecology Section Archive
"#;

        let key = parse_windows_install_registry_key_from_search_output(
            output,
            "ecology-section-archive.exe",
            Some(Path::new(
                r"C:\Program Files\Ecology Section Archive\ecology-section-archive.exe",
            )),
        )
        ...
        assert_eq!(
            key,
            r"HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Uninstall\EcologySectionArchive"
        );
    }
...
HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Uninstall\EcologySectionArchive
    DisplayIcon    REG_SZ    C:\Program Files\Ecology Section Archive\ecology-section-archive.exe,0
    InstallLocation    REG_SZ    C:\Program Files\Ecology Section Archive
    QuietUninstallString    REG_SZ    "C:\Program Files\Ecology Section Archive\uninstall.exe" /S
    UninstallString    REG_SZ    "C:\Program Files\Ecology Section Archive\uninstall.exe"
"#;
        ...
        assert_eq!(
            record.install_dir,
            Some(PathBuf::from(r"C:\Program Files\Ecology Section Archive"))
        );
        assert_eq!(
            record.quiet_uninstall_command.as_deref(),
            Some(r#""C:\Program Files\Ecology Section Archive\uninstall.exe" /S"#)
        );
        assert_eq!(
            record.uninstall_command.as_deref(),
            Some(r#""C:\Program Files\Ecology Section Archive\uninstall.exe""#)
        );
    }
    ...
    fn builds_silent_windows_uninstall_command_from_registry_record() {
        let nsis_record = WindowsRegistryInstallRecord {
            key_path: "HKLM\\Software\\Ecology".into(),
            launch_target: Some(PathBuf::from(
                r"C:\Program Files\Ecology Section Archive\ecology-section-archive.exe",
            )),
            install_dir: Some(PathBuf::from(r"C:\Program Files\Ecology Section Archive")),
            quiet_uninstall_command: None,
            uninstall_command: Some(
                r#""C:\Program Files\Ecology Section Archive\uninstall.exe""#.into(),
            ),
        };
        let msi_record = WindowsRegistryInstallRecord {
            key_path: "HKLM\\Software\\EcologyMsi".into(),
            launch_target: Some(PathBuf::from(
                r"C:\Program Files\Ecology Section Archive\ecology-section-archive.exe",
            )),
            install_dir: Some(PathBuf::from(r"C:\Program Files\Ecology Section Archive")),
    ...
    fn mounted_dmg_selection_prefers_root_app_bundle() {
        ...
        let root_bundle = mounted_root.join("生态科档案.app");
        let stale_bundle = fake_applications
            .join(".ecology-section-archive-update-stage-123")
            .join("生态科档案.app");
        ...
        let selected =
            select_app_bundle_from_mounted_dmg(&mounted_root, Some("生态科档案.app"), &mut log)
    ...
    fn bundle_search_skips_symlink_dirs_and_update_stage_dirs() {
        ...
        let valid_bundle = extracted_root.join("Nested").join("生态科档案.app");
        let stage_bundle = extracted_root
            .join(".ecology-section-archive-update-stage-123")
            .join("生态科档案.app");
        let symlink_target = root.join("linked");
        let symlink_bundle = symlink_target.join("生态科档案.app");
