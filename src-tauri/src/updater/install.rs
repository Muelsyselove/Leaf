...
const UPDATE_HELPER_PATH_ENV: &str = "ECOLOGY_SECTION_ARCHIVE_UPDATE_HELPER_PATH";
const UPDATE_HELPER_MODE_ENV: &str = "ECOLOGY_SECTION_ARCHIVE_UPDATE_HELPER_MODE";
...
fn stage_windows_helper_copy(paths: &UpdatePaths, source_path: &Path) -> Result<PathBuf, AppError> {
    ...
    let source_ext = source_path
        ...
        .and_then(|ext| ext.to_str())
        .unwrap_or("exe");
    let staged_exe = staged_dir.join(format!("ecology-update-helper.{source_ext}"));
...
mod tests {
    ...
    fn launches_real_install_helper_after_staging_copy() {
        ...
        let bundle = paths.root_dir().join("Ecology Section Archive.app");
    ...
    fn resolves_macos_helper_and_target_from_fallback_search_dir() {
        ...
        let actual_bundle = fallback_dir.join("生态科档案.app");
        write_bundle_with_helper(&actual_bundle);

        let stale_bundle = paths.root_dir().join("gone").join("生态科档案.app");
    ...
    fn stages_windows_helper_with_adjacent_dll_dependencies() {
        ...
        let source = source_dir.join("ecology-section-archive.exe");
        ...
        assert_eq!(
            ...
            Some("ecology-update-helper.exe")
        );
