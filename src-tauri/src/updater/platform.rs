...
mod tests {
    ...
    fn detects_windows_portable_installation() {
        let install_kind = detect_install_kind(
            Os::Windows,
            Some(Path::new(r"D:\Apps\Ecology\ecology-section-archive.exe")),
        );
    ...
    fn detects_macos_app_bundle() {
        let bundle = find_macos_app_bundle(Path::new(
            "/Applications/Ecology Section Archive.app/Contents/MacOS/ecology-section-archive",
        ));

        assert_eq!(
            bundle,
            Some(PathBuf::from("/Applications/Ecology Section Archive.app"))
        );
        assert_eq!(
            detect_install_kind(
                Os::Macos,
    ...
    fn infers_macos_aarch64_dmg() {
        let asset = infer_asset_from_filename(
            "ecology-section-archive_1.0.5_aarch64.dmg",
    ...
    fn infers_windows_x64_msi() {
        let asset = infer_asset_from_filename(
            "ecology-section-archive_1.0.5_x64_en-US.msi",
    ...
    fn infers_windows_x64_setup_exe() {
        let asset = infer_asset_from_filename(
            "ecology-section-archive_1.0.5_x64-setup.exe",
    ...
    fn treats_unbundled_macos_binary_as_unknown_install_kind() {
        let install_kind = detect_install_kind(
            Os::Macos,
            Some(Path::new("/Users/test/dev/ecology-section-archive")),
        );
